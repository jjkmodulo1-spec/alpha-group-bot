...
ALARM_CHECKPOINTS: list[int] = [604800, 259200, 172800, 86400, 21600, 3600, 600]  # 7d, 3d, 2d, 24h, 6h, 1h, 10min
...
ALARM_CONFIDENCE_THRESHOLD = float(os.getenv("ALARM_CONFIDENCE_THRESHOLD", "0.75"))


def _gemini_extract_time(text: str, now_str: str, now_unix: int) -> dict:
    """Focused Gemini call: extract ONLY the fire timestamp from the text.

    Having a single responsibility means the model isn't distracted trying to
    also summarise an event name — it just nails the time.
    """
    prompt = (
        "You extract the exact date and time from crypto group announcements.\n"
        f"Current time: {now_str}  (Unix: {now_unix})\n\n"
        "Task: find when the event fires and return a Unix timestamp.\n\n"
        "Rules:\n"
        "- Prefer explicitly stated times over implied ones.\n"
        "- If no timezone is stated, assume UTC.\n"
        "- Times in 24h format (hour ≥ 13) are already PM — do NOT add 12.\n"
        "- 'tomorrow' = next calendar day at the stated time (09:00 UTC if none given).\n"
        "- 'tonight' = today at 20:00 UTC if no time given.\n"
        "- 'in X hours/minutes/days' → add to current Unix timestamp exactly.\n"
        "- Dates with no year: use nearest future occurrence.\n"
        ...
        '{"fire_at_unix":<integer or null>,"confidence":<float>,"error":<null or "reason">}'
    )
...
def _gemini_extract_label(text: str) -> str | None:
    """Focused Gemini call: extract ONLY the event label from the text.

    Not distracted by timestamps — just names the event clearly.
    """
    prompt = (
        "You write short event names for crypto group alarm reminders.\n\n"
        "Task: read the text below and return a short, clear event name (max 60 chars).\n\n"
        "Rules:\n"
        "- Keep token names, project names, ticker symbols, and Twitter/Telegram handles.\n"
        "- Drop filler: 'Details:', 'from their official manifest', 'Collection Page:', etc.\n"
        "- Include the event type: mint, airdrop, space, listing, snapshot, launch, AMA, etc.\n"
        "- Do NOT include dates, times, prices, supply numbers, or URLs.\n\n"
        "Examples:\n"
        '  Input: "@TheSymbioteLab \'s mint is on Ethereum. Date: May 19, 2026 WL: 0.002 ETH at 13:00 UTC"\n'
        '  Output: {"label":"@TheSymbioteLab WL Mint"}\n\n'
        '  Input: "CryptoAlpha Twitter space tonight 8pm EST — join us!"\n'
        '  Output: {"label":"CryptoAlpha Twitter Space"}\n\n'
        '  Input: "$TOKEN listing on Binance May 20 at noon, huge volume expected"\n'
        '  Output: {"label":"$TOKEN Binance Listing"}\n\n'
        '  Input: "BAYC airdrop claim in 2 hours — snapshot already done"\n'
        '  Output: {"label":"BAYC Airdrop Claim"}\n\n'
        f'Text:\n"""\n{text}\n"""\n\n'
        "Return ONLY valid JSON, no markdown:\n"
        '{"label":"<event name>"}'
    )
    ...
    with urllib.request.urlopen(req, timeout=GEMINI_TIMEOUT_SECONDS) as resp:
        ...
        return str(label).strip()[:200] if label else None


def parse_alarm_with_gemini(raw_text: str) -> dict | None:
    """Parse a natural-language alarm request using two parallel focused Gemini calls.

    One call extracts the timestamp, one extracts the label. Each has a single
    responsibility and few-shot examples — far more accurate than one call doing both.
    ...
    # Run both extractions concurrently in threads (both are blocking HTTP calls)
    import concurrent.futures
    with concurrent.futures.ThreadPoolExecutor(max_workers=2) as pool:
        time_future = pool.submit(_gemini_extract_time, normalised, now_str, now_unix)
        label_future = pool.submit(_gemini_extract_label, normalised)

        try:
            time_result = time_future.result(timeout=GEMINI_TIMEOUT_SECONDS + 2)
        except Exception as e:
            logger.error(f"Gemini time extraction failed: {e}")
            return None

        try:
            label = label_future.result(timeout=GEMINI_TIMEOUT_SECONDS + 2)
        except Exception as e:
            logger.warning(f"Gemini label extraction failed: {e}")
            label = None

    confidence = safe_float(time_result.get("confidence")) or 0.0
    fire_at_raw = time_result.get("fire_at_unix")

    if fire_at_raw is None or confidence < ALARM_CONFIDENCE_THRESHOLD:
        logger.info(
            f"Alarm time parse failed: confidence={confidence:.2f} "
            f"fire_at={fire_at_raw} error={time_result.get('error')!r}"
        )
        return None
    ...
    # Fall back to a truncated version of the raw input if label extraction failed
    if not label:
        label = raw_text[:80].strip()

    parsed = {"label": label, "fire_at": fire_at, "confidence": confidence}
