...
def load_config():
    """Read from disk once at startup to populate the cache."""
    global _config
...
def track_chain(user, chat_id: int, msg_id: int, partner_id: int) -> list:
    ...
    if len(conversation_chains[pair]["messages"]) >= CHAIN_THRESHOLD:
        to_delete = [(m[0], m[1], m[2]) for m in conversation_chains[pair]["messages"]]
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Fast path: explicit reply to someone else
    ...
    if partner_id is not None:
        ...
        if to_delete:
            logger.info(f"Chain hit. Deleting {len(to_delete)} messages.")
...
def main():
    load_config()  # Populate cache once at startup
